...
#define nullptr NULL
	...
	void DockContext::SaveLayout(JSON_File * json, const char* layout_name)
	{
		json->AddObject(layout_name);
		json->ChangeObject(layout_name);
		...
		for (int i = 0; i < m_docks.size(); ++i) 
		{
			...
			json->SetString((di + "ID").c_str(), std::to_string(i).c_str());
			json->SetString((di + "Label").c_str(), dock.parent ? (dock.label[0] == '\0' ? "DOCK" : dock.label) : dock.status == Status_Float ? dock.label : "ROOT");
			json->SetNumber((di + "pos_X").c_str(), (int)dock.pos.x);
			json->SetNumber((di + "pos_Y").c_str(), (int)dock.pos.y);
			json->SetNumber((di + "size_X").c_str(), (int)dock.size.x);
			json->SetNumber((di + "size_Y").c_str(), (int)dock.size.y);
			json->SetNumber((di + "status").c_str(), (int)dock.status);
			json->SetBool((di + "active").c_str(), dock.active);
			json->SetBool((di + "opened").c_str(), dock.opened);
			fillLocation(dock);
			json->SetString((di + "location").c_str(), strlen(dock.location) ? dock.location : "-1");
			json->SetNumber((di + "child0").c_str(), getDockIndex(dock.children[0]));
			json->SetNumber((di + "child1").c_str(), getDockIndex(dock.children[1]));
			json->SetNumber((di + "prev_tab").c_str(), getDockIndex(dock.prev_tab));
			json->SetNumber((di + "next_tab").c_str(), getDockIndex(dock.next_tab));

			if (dock.parent == nullptr && !dock.opened)
			{
				json->SetNumber((di + "parent").c_str(), -1);
			}
			else if (dock.parent == nullptr && &dock != getRootDock()) 
			{
				json->SetNumber((di + "parent").c_str(), getDockIndex(getRootDock()));
			}
			else 
			{
				json->SetNumber((di + "parent").c_str(), getDockIndex(dock.parent));
			}
		}
	...
	bool DockContext::LoadLayout(JSON_File * json, const char* layout_name)
	{
		bool ret = true;
		...
		if (json->ChangeObject(layout_name))
		{
