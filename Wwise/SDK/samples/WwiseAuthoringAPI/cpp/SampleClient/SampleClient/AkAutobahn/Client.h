...
namespace AK
{
	namespace WwiseAuthoringAPI
	{
		...
		public:
			...
			bool Call(const char* in_uri, const char* in_args, const char* in_options, std::string& out_result);
			bool Call(const char* in_uri, const AkJson& in_args, const AkJson& in_options, AkJson& out_result);
		...
		private:
			...
			template<typename T> bool GetFuture(std::future<T>& in_value, T& out_result)
			{
				if (m_timeout > 0)
				{
					return GetFutureWithTimeout(m_timeout, in_value, out_result);
				}
				else
				{
