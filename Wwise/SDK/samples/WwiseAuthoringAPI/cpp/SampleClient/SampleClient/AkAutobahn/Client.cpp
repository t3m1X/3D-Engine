	...
	namespace WwiseAuthoringAPI
	{
		...
		bool Client::Connect(const char* in_uri, unsigned int in_port)
		{
			try
			{
				...
				uint64_t result;
				return GetFuture<uint64_t>(future, result);
			}
		...
		bool Client::Call(const char* in_uri, const char* in_args, const char* in_options, std::string& out_result)
		{
			try
			{
				AkJson jsonArgs;
				...
				bool success = GetFuture<AkJson>(future, result);
		...
		bool Client::Call(const char* in_uri, const AkJson& in_args, const AkJson& in_options, AkJson& out_result)
		{
			try
			{
				auto future = m_ws->call_options(in_uri, std::vector<AkVariant>{}, in_args, in_options);
				return GetFuture<AkJson>(future, out_result);
			}
			catch (std::exception& e)
			{
				Log(e.what());
		...
		bool Client::SubscribeImpl(const char* in_uri, const AkJson& in_options, handler_t in_callback, uint64_t& out_subscriptionId)
		{
			...
			if (!GetFuture<subscription>(future, resultObject))
			{
				return false;
			}
		...
		bool Client::UnsubscribeImpl(const uint64_t& in_subscriptionId)
		{
			...
			AkJson output;
			return GetFuture<AkJson>(result, output);
		}
