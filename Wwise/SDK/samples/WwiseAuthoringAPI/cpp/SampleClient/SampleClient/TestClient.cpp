...
void TestWampClientJsonString()
{
...

	{
...

		{
			...
			if (doc.Parse(queryResultString.c_str()).HasParseError() || !JSONHelpers::FromRapidJson(doc, queryResult))
			{
...

		{
			...
			rapidjson::Document doc;
			if (doc.Parse(createResultString.c_str()).HasParseError() || !JSONHelpers::FromRapidJson(doc, createResult))
			{
				std::cout << "Failed to parse result JSON string." << std::endl;
				return;
			}
